 ...
 */
export async function findGenerationByFileId(
  fileId: string
): Promise<string | null> {
  try {
 ...
 */
export async function getFileIdsFromGeneration(
  generationId: string
): Promise<string[]> {
  try {
 ...
 */
export function getFileStatusUrl(fileId: string, locale = "en"): string {
 ...
 */
export function getGenerationStatusUrl(
  generationId: string,
  locale = "en"
): string {
...
export function isValidGenerationId(str: string): boolean {
  return (
    str.startsWith("veo3_") ||
    str.startsWith("api_test_") ||
    str.startsWith("sunset_test_")
  );
}
 ...
 */
export function getFileTypeFromResponse(
