...
export async function POST(req: NextRequest) {
  try {
    ...
    const {
      ...
      promptData,
      moodboard,
    } = enhancePromptSchema.parse(body);
    ...
    if (moodboard?.enabled && moodboard.images?.length > 0) {
      ...
      const tagDescriptions = {
        ...
        background:
          "environments, settings, locations, and background elements",
        lighting:
    ...
    if (hasCharacterSpeech && promptData?.characters) {
      const charactersWithSpeech = promptData.characters.filter((char) =>
        char.speech.trim()
      );
...
LIGHTING/MOOD: ${structuredData.lighting_mood}`;
    ...
    if (error instanceof Error && error.message?.includes("content filter")) {
      return NextResponse.json(
        {
          error:
            "Content was filtered. Please modify your prompt and try again.",
        },
        { status: 400 }
      );
    }
