...
async function testImageToImageGeneration() {
  if (!SUPERDUPERAI_TOKEN) {
    ...
    console.log('✅ Smoke test passed - image-to-image payload structure is valid');
    console.log('📋 Summary:');
  ...
  try {
    ...
    const payload = {
      ...
      generationType: "image-to-image",
      config: {
        prompt: "Transform this image into a beautiful watercolor painting with soft colors",
        shot_size: "Medium Shot",
        style_name: "flux_watercolor",
        seed: "35915533265",
        aspecRatio: "1:1",
...
async function testImageToImageModels() {
  ...
  const availableModels = [
    ...
    { id: 'comfyui/stable-diffusion', label: 'Stable Diffusion', supportsImageToImage: true },
    { id: 'comfyui/dalle', label: 'DALL-E', supportsImageToImage: false }
  ];
  
  const imageToImageModels = availableModels.filter(model => model.supportsImageToImage);
