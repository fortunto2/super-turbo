  ...
  return new Promise((resolve) => {
    ...
    ws.on('open', () => {
      ...
      subscriptions.forEach((sub, index) => {
        setTimeout(() => {
          console.log(
            `📤 ${endpoint} - Sending subscription ${index + 1}:`,
            sub,
          );
    ...
    ws.on('message', (data) => {
      ...
      if (message.type === 'subscribe') {
        ...
        if (message.object?.url) {
          console.log(
            `🎉 ${endpoint} - SUCCESS! File completed with URL:`,
            message.object.url,
          );

          // Check if this is our target file
          if (
            message.fileId === targetFileId ||
    ...
    ws.on('close', (code, reason) => {
      console.log(
        `🔌 ${endpoint} - Closed (${code}): ${reason || 'no reason'}`,
      );
      clearTimeout(timeout);
