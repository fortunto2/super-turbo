...
async function testVideoCompatibility() {
  ...
  try {
    ...
    const { generateVideoHybrid } = await import(
    ...
    console.log('🎬 ✅ Image-to-Video mode:', {
      ...
      hasSourceImage: !!(
        imageToVideoParams.sourceImageId || imageToVideoParams.sourceImageUrl
      ),
    });
...
async function testAPIRoutesCompatibility() {
  ...
  try {
    ...
    console.log(
      '🔌 ✅ Image API accepts all required fields:',
      imageAPIFields.join(', '),
    );
    console.log(
      '🔌 ✅ Video API accepts all required fields:',
      videoAPIFields.join(', '),
    );
    console.log(
      '🔌 ✅ Both chat artifacts and standalone tools can use these fields',
    );
...
async function testGenerationTypeDetection() {
  ...
  try {
    ...
    for (const testCase of testCases) {
      const detected =
        testCase.sourceImageId || testCase.sourceImageUrl
          ? 'image-to-video'
          : 'text-to-video';
      const passed = detected === testCase.expected;

      console.log(
        `🎯 ${passed ? '✅' : '❌'} ${testCase.description}: ${detected} (expected: ${testCase.expected})`,
      );

      if (!passed) {
        throw new Error(
          `Generation type detection failed for: ${testCase.description}`,
        );
      }
    }
    ...
    console.error(
      '🎯 ❌ Generation type detection test failed:',
      error.message,
    );
    return false;
  }
...
export {
  ...
  testAPIRoutesCompatibility,
  testGenerationTypeDetection,
};
